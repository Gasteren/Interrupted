...
SML:Register("font", "NC_TeenBold", "Interface\\AddOns\\Interrupted\\media\\teen_bold.ttf", 255);
...
do
	...
	function OnStartup()
		...
		SLASH_Interrupted1 = '/interrupted';
		SlashCmdList["Interrupted"] = function(msg, editBox)
	...
	function InitializeDB()
		if (InterruptedDB[LocalPlayerFullName] == nil) then
			InterruptedDB[LocalPlayerFullName] = { };
		end
		local defaults = {
			CDsTable = { },
			IconSize = 26,
		...
		for key, value in pairs(defaults) do
			if (InterruptedDB[LocalPlayerFullName][key] == nil) then
				InterruptedDB[LocalPlayerFullName][key] = value;
			end
		end
		db = InterruptedDB[LocalPlayerFullName];
	end
		
	function AddButtonToBlizzOptions()
		local frame = CreateFrame("Frame", "NC_BlizzOptionsFrame", UIParent);
		frame.name = "Interrupted";
		InterfaceOptions_AddCategory(frame);
		local button = GUICreateButton("NC_BlizzOptionsButton", frame, "/interrupted");
		button:SetWidth(80);
		button:SetHeight(40);
...
do
	...
	function AllocateIcon(frame)
		...
		texture.border:SetTexture("Interface\\AddOns\\Interrupted\\media\\CooldownFrameBorder.tga");
...
do
	...
	function GUICategory_2(index, value)
		...
		buttonCopyProfile:SetScript("OnClick", function(self, ...)
			if (dropdownCopyProfile.myvalue ~= nil) then
				InterruptedDB[LocalPlayerFullName] = deepcopy(InterruptedDB[dropdownCopyProfile.myvalue]);
				db = InterruptedDB[LocalPlayerFullName];
		...
		buttonDeleteProfile:SetScript("OnClick", function(self, ...)
			if (dropdownDeleteProfile.myvalue ~= nil) then
				InterruptedDB[dropdownDeleteProfile.myvalue] = nil;
	...
	function GUICategory_Other(index, value)
		...
		scrollAreaBackground:SetBackdrop({
			bgFile = "Interface\\AddOns\\Interrupted\\media\\Smudge.tga",
			edgeFile = "Interface\\AddOns\\Interrupted\\media\\Border",
	...
	function RebuildDropdowns()
		...
		local initCopyProfile = function()
			wipe(info);
			for index in pairs(InterruptedDB) do
...
do
	...
	function Print(...)
		...
		DEFAULT_CHAT_FRAME:AddMessage(format("Interrupted: %s", text), 0, 128, 128);
	end
